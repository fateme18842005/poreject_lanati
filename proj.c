...
int run_init(int argc , char *const argv[]){
    ...
    do{
        DIR *dir = opendir(".");
        if(dir == NULL){
            perror("Error opening current dire ctory");
            return 1;
        }
        while ((entry = reddir(dir)) != NULL){
            if(entry->d_type == DT_DIR && strcmp(entry->d_name , ".gitmn") == 0)
            exists = true;
        }
        closedir(dir);
    ...
    if(!exists){
        if(mkdir(".gitmn" , 0755) != 0 ){
            return 1;
        }
        return create_configs("fatemeh" , "fatamehka0994@gmail.com");
    }
    else{
        perror("gitmnrepository has already initialized");
    }
...
int create_configs(char *username , char *email){
    FILE *file = fopen(".gitmn/config" , "w");
    if(file == NULL){
        return 1;
    }
    ...
    fclose(file);
    if(mkdir(".gitmn/commits" , 0755) != 0){
        return 1;
    }
    if(mkdir(".gitmn/files" , 0755) != 0){
        return 1;
    }
    file = fopen(".gitmn/staging" , "w");
    fclose(file);
    file = fopen(".gitmn/trachs" , "w");
    fclose(file);
    return 0;
...
int add_to_staging(char *filepath) {
    FILE *file = fopen(".gitmn/staging", "r");
...
int inc_last_commit_ID() {
    FILE *file = fopen(".gitmn/config", "r");
    if (file == NULL) return -1;
    
    FILE *tmp_file = fopen(".gitmn/tmp_config", "w");
    ...
    remove(".gitmn/config");
    rename(".gitmn/tmp_config", ".gitmn/config");
    return last_commit_ID;
}

bool check_file_directory_exists(char *filepath) {
    DIR *dir = opendir(".gitmn/files");
    struct dirent *entry;
    if (dir == NULL) {
...
bool is_tracked(char *filepath) {
    FILE *file = fopen(".gitmn/tracks", "r");
...
int create_commit_file(int commit_ID, char *message) {
    ...
    strcpy(commit_filepath, ".gitmn/commits/");
    char tmp[10];
    sprintf(tmp, "%d", commit_ID);
...
int find_file_last_commit(char* filepath) {
    ...
    strcpy(filepath_dir, ".gitmn/files/");
